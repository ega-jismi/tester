      ...
      {orders.length === 0 ? (
        <div className="text-center py-20 bg-slate-50 dark:bg-slate-800 rounded-xl border border-dashed border-slate-300 dark:border-slate-700">
          ...
          <p className="text-slate-500 mb-6">
            Yuk mulai belanja buku favoritmu!
          </p>
          <Link
            href="/katalog"
            className="px-6 py-2 bg-bookBlue text-white rounded-lg font-bold"
          >
            Ke Katalog
          </Link>
        </div>
          ...
          {orders.map((order) => (
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 pb-4 border-b border-slate-100 dark:border-slate-700">
                ...
                <div
                  className={`mt-2 sm:mt-0 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-2 ${getStatusColor(order.status)}`}
                >
              ...
              <div className="flex justify-between items-center pt-4 border-t border-slate-100 dark:border-slate-700">
                <div>
                  ...
                  <p className="text-lg font-bold text-bookOrange">
                    Rp {order.total.toLocaleString("id-ID")}
                  </p>
                </div>

                {/* === UPDATE DI SINI: GANTI BUTTON DENGAN LINK === */}
                <Link
                  href={`/orders/${order.id}`}
                  className="px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg text-sm font-bold hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors text-slate-700 dark:text-slate-200"
                >
                  Lihat Detail
                </Link>
                {/* ============================================== */}
